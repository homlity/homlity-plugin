...
/**
 * Builder switchers arrive as 'yes'/'' (Elementor, and the Divi/WPBakery
 * compatibility layers normalize to the same pair). Anything absent keeps the
 * default so the plugin template and the PDF render the full sheet.
 */
$sheetShow = static function (string $key, bool $default) use ($settings): bool {
    ...
    return in_array(strtolower(trim((string) $value)), ['yes', 'on', 'true', '1'], true);
};

// The address is commercially sensitive: it is opt-in on the sheet.
$showAddress     = $sheetShow('show_address', false);
$showHero        = $sheetShow('show_hero', true);
$showActions     = $sheetShow('show_actions', true);
$showAdvisor     = $sheetShow('show_advisor', true);
$showFinance     = $sheetShow('show_finance', true);
$showInfo        = $sheetShow('show_info', true);
$showDimensions  = $sheetShow('show_dimensions', true);
$showDescription = $sheetShow('show_description', true);
$showFeatures    = $sheetShow('show_features', true);
$showMedia       = $sheetShow('show_media', true);
$showLegal       = $sheetShow('show_legal', true);
...
// A widget colour wins over the global plugin colour. It is written inline
// instead of through a builder selector because the CSS custom properties of
// the sheet live on this very element.
$primaryColor = sanitize_hex_color((string) ($settings['sheet_primary'] ?? ''))
    ?: (sanitize_hex_color((string) ($pluginSettings['primary_color'] ?? '')) ?: '#ff6752');
...
if ($showAddress) {
    array_splice($infoItems, 1, 0, [['Dirección', get_post_meta($post_id, $meta['address'], true)]]);
}
