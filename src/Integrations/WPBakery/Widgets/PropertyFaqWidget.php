...
use Homlity\PluginInmobiliario\Integrations\WPBakery\Compatibility\Repeater;
...
class PropertyFaqWidget extends BasePropertyWidget
{
    ...
    private function resolve_post_id(array $settings): int
    {
        if (($settings['use_current'] ?? 'yes') === 'yes') {
            return $this->current_property_id();
        }
    ...
    {
        if (!empty($icon['value'])) {
            \Homlity\PluginInmobiliario\Integrations\WPBakery\Compatibility\Icons_Manager::render_icon($icon, $attrs);
        }
    }

    protected function render(): void
    {
        \Homlity\PluginInmobiliario\Integrations\WPBakery\Compatibility\Icons_Manager::enqueue_shim();
        ...
        $generator = new PropertyFaqGenerator($post_id);
