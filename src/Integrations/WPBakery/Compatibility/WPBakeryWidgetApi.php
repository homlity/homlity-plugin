...
namespace Homlity\PluginInmobiliario\Integrations\WPBakery\Compatibility;
...
if (!class_exists(Widget_Base::class)) {
    abstract class Widget_Base
    {
        ...
        private string $homlityId;
        private static int $homlityInstanceCounter = 0;

        public function __construct(array $data = [], array $args = [])
        {
            self::$homlityInstanceCounter++;
            $this->homlityId = substr(md5(static::class . ':' . self::$homlityInstanceCounter), 0, 8);
        ...
        public function homlityIsEditor(): bool
        {
            ...
            return str_starts_with($action, 'vc_')
    ...
    final class Icons_Manager
    {
        ...
        public static function render_icon(array $icon, array $attributes = []): void
        {
            $classes = array_values(array_filter(array_map(
                'sanitize_html_class',
                preg_split('/\s+/', trim((string) ($icon['value'] ?? ''))) ?: []
            )));
            if ($classes === []) {
                return;
            }
