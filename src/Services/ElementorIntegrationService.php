...
class ElementorIntegrationService implements ServiceInterface
{
    ...
    public function register(): void
    {
        ...
        add_action('init', [$this, 'disableCoreRevisionForElementorSave'], PHP_INT_MAX);
    ...
    /**
     * Core's revision callback loads every revision for the document after an
     * Elementor save. Large Elementor documents with a long history can make
     * that single query consume hundreds of megabytes. Skip only the core
     * revision generated by save_builder; no existing revision is deleted.
     */
    public function disableCoreRevisionForElementorSave(): void
    {
        if (!$this->isElementorSaveRequest()) {
            return;
        }
    ...
    private function isElementorSaveRequest(): bool
    {
        ...
        return is_string($actions)
            && preg_match('/"action"\s*:\s*"save_builder"/', $actions) === 1;
    }
